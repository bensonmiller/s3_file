...
# This module provides helper methods for authenticating with S3.
module S3Credentials
  ...
  def self.get_s3_credentials(aws_access_key_id, aws_secret_access_key, token)
    if aws_access_key_id.nil? && aws_secret_access_key.nil? && token.nil?
      return credentials_from_instance_profile
    else
  ...
  def self.credentials_from_instance_profile
    ...
    { :token => token, :aws_access_key_id => aws_access_key_id, :aws_secret_access_key => aws_secret_access_key }
  end
end
